...
无需 App Store，30 秒完成安装。安装后可离线使用，数据保存在手机本地。

### iPhone / iPad

1. 用 **Safari** 浏览器打开 `https://giantsand26.github.io/keepfit/`
   > ⚠️ 必须用 Safari，Chrome 不支持添加到主屏幕
2. 点底部工具栏中间的 **分享按钮**（方框带向上箭头 ▢↑）
3. 向下滑动菜单，找到并点 **「添加到主屏幕」**（Add to Home Screen）
4. 名称保持「居家训练」或自己改名 → 点右上角 **「添加」**
5. 回到主屏幕，看到 🏋️ 图标 → 点开即用

### Android

1. 用 **Chrome** 浏览器打开 `https://giantsand26.github.io/keepfit/`
2. 点右上角 **⋮** 菜单
3. 点 **「添加到主屏幕」**（Add to Home Screen）
4. 点 **「添加」** 确认
5. 主屏幕出现图标 → 点开即用

### 安装后特性

- ✅ 像原生 App 一样全屏运行（无浏览器地址栏）
- ✅ 完全离线可用（断网也能看训练计划）
- ✅ 数据保存在手机本地，不上传任何服务器
- ✅ 组间休息结束自动震动提醒
